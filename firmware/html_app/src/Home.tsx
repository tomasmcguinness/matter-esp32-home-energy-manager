...
type SimpleDevice = { nodeId: number; endpointId: number; label: string; hasElectricalSensor: boolean }
    ...
    fetch('/api/nodes')
      ...
      .then((data: { nodes: SavedNode[] }) => {
        ...
        if (gm?.settings?.nodeId !== undefined && gm.settings.endpointId !== undefined && gm.settings.label) {
          setGridSensor({ nodeId: gm.settings.nodeId, endpointId: gm.settings.endpointId, label: gm.settings.label, hasElectricalSensor: true })
        }
      })
      .catch(() => { })
  }, [])
