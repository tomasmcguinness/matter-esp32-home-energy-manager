...
export const handlers = [
  ...
  powerWs.addEventListener('connection', ({ client }) => {
    ...
    // node power + edge flow animate. The battery value sweeps through zero so
    // charge/discharge direction flips.
    let tick = 0
    const entry = (endpointId: number, value: number, clusterId = 144, attributeId = 0x08) =>
      ({ nodeId: 10000, endpointId, clusterId, attributeId, value })
    const interval = setInterval(() => {
      tick++
      client.send(JSON.stringify({
        type: 'attribute_batch',
        data: [
          entry(13482, Math.round(1500000 + Math.random() * 600000)),   // PV String 1 DC, W·1000
